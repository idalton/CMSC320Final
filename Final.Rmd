...
start_lines <- c(grep("Alabama .", text)) # " ." added for increased accuracy
# I included the space here as well just to be safe
end_lines <- c(grep("Wyoming .", text))
...
for (index in 1:length(start_lines)){
    ...
    table <- table %>% 
      ...
      rbind(c("AK",rep("0",7))) %>%
      drop_na()
      # complete(-state, fill= list(rep(0))) 
...
masterhoneydf %>%
  ...
  # aka I didnt think GOLD looked honey enough
...
#   group_by(state) %>%
#   summarize(avg_numcol = mean(numcolonies)) %>%
#   ggplot(aes(map_id = abbr_to_name[state])) + 
...
p <- ggplot(yolo,aes(map_id = str_to_lower(abbr_to_name[state]))) + 
  # map points to the fifty_states shape data
  geom_map(aes(fill = avg_numcol), map = fifty_states) + 
  expand_limits(x = fifty_states$long, y = fifty_states$lat) +
...
Predicting the change in number of colonies in 2012
